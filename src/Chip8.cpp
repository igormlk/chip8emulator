...
void Chip8::operator()()
{
    while( !testeAtomic->quit.load() ){
        /*gpu->setPixels(static_cast<Uint32>(rand() % 255));
        ...
        boost::this_thread::sleep_for(boost::chrono::milliseconds{clock});
    }
}

void Chip8::setGpu(Gpu * pGpu)
{
    gpu = pGpu;
}
...
void Chip8::setAtomic(TesteAtomic *pAtomic)
{
    this->testeAtomic = pAtomic;
}
